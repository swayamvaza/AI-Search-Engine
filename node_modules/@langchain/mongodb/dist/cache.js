...
var MongoDBCache = class extends BaseCache {
	...
	async lookup(prompt, llmKey) {
		...
		try {
			return JSON.parse(doc[this.RETURN_VAL]).map((g) => deserializeStoredGeneration(JSON.parse(g)));
		} catch {
			return null;
		}
	}
...
var MongoDBAtlasSemanticCache = class MongoDBAtlasSemanticCache extends BaseCache {
	...
	async lookup(prompt, llmString) {
		const searchQuery = {
			queryVector: MongoDBAtlasSemanticCache.fixArrayPrecision(await this.getEmbedding(prompt)),
